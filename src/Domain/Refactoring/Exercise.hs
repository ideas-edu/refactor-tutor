...
makeRefactorExercise refExInput settings = addReady $ emptyExercise
      { -- identification and meta-information
        ...
        extraRules     = [use buggyEqualsTrue, use buggyCollapseIfR, use incrementAssignBuggy, use compoundSubtractionBuggy, use decrementAssignBuggy, use compoundAdditionBuggy, use removeIfEqualsLitViewBuggy]
      }
    where
